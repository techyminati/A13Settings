...
public class DeviceInfoUtils {
    ...
    private static String getRawPhoneNumber(Context context, int i) {
        if (BuildCompat.isAtLeastT()) {
            return getRawPhoneNumberFromT(context, i);
        }
        return ((TelephonyManager) context.getSystemService(TelephonyManager.class)).createForSubscriptionId(i).getLine1Number();
    }

    private static String getRawPhoneNumberFromT(Context context, int i) {
        return ((SubscriptionManager) context.getSystemService(SubscriptionManager.class)).getPhoneNumber(i);
    }
}
