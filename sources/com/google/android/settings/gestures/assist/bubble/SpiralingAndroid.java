...
public class SpiralingAndroid {
    ...
    public SpiralingAndroid(Context context, Rect rect) {
        ...
        this.mRotationSpeed = ((this.mRandom.nextFloat() * 0.8f) + 0.8f) * 360.0f;
        int width = (rect.width() / 10) + this.mRandom.nextInt(rect.width() / 5);
        Drawable newDrawable = context.getResources().getDrawable(R.drawable.ic_android_black_24dp, null).getConstantState().newDrawable();
