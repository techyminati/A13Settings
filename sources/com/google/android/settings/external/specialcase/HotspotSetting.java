...
public class HotspotSetting implements Queryable {
    ...
    public Cursor getAccessCursor(Context context, SliceData sliceData) {
        return createQueryCursor(getCurrentValue((WifiManager) context.getSystemService("wifi")), getAvailability(context, (ConnectivityManager) context.getSystemService("connectivity")), getIntentString(context, "enable_wifi_ap", TetherSettings.class, getScreenTitle(context), R.string.menu_key_network), getIconResource());
    }
    ...
    public Cursor getUpdateCursor(Context context, SliceData sliceData, int i) {
        ...
        String intentString = getIntentString(context, "enable_wifi_ap", TetherSettings.class, getScreenTitle(context), R.string.menu_key_network);
    ...
    private int getAvailability(Context context, ConnectivityManager connectivityManager) {
        ...
        return connectivityManager.isTetheringSupported() ? 0 : 2;
    }

    private void updateHotspot(ConnectivityManager connectivityManager, int i) {
        if (i == 1) {
            connectivityManager.startTethering(0, true, new ConnectivityManager.OnStartTetheringCallback() { // from class: com.google.android.settings.external.specialcase.HotspotSetting.1
            });
        } else {
