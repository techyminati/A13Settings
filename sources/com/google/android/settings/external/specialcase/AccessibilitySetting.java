...
public class AccessibilitySetting implements Queryable {
    ...
    public Cursor getAccessCursor(Context context, SliceData sliceData) {
        ...
        String intentString = getIntentString(context, this.mServiceName, AccessibilitySettings.class, getScreenTitle(context), R.string.menu_key_accessibility);
    ...
    private int getCurrentValue(Context context) {
        return (!(Settings.Secure.getInt(context.getContentResolver(), "accessibility_enabled", 0) == 1) || !AccessibilityUtils.getEnabledServicesFromSettings(context).contains(this.mComponentName)) ? 0 : 1;
    }
