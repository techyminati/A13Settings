...
public class PsdBundle {
    private static final String[] TELEPHONY_PROJECTION = {"KEY", "VALUE"};
    ...
    public static class Builder {
        ...
        public Builder addTopBatteryApps() {
            ...
            double d = consumedPower / dischargePercentage;
        ...
        public static /* synthetic */ int lambda$addTopBatteryApps$0(UidBatteryConsumer uidBatteryConsumer, UidBatteryConsumer uidBatteryConsumer2) {
        ...
        public Builder addConnectedBluetoothDevicesSignals() {
            ...
            for (BluetoothDevice bluetoothDevice : bondedBtDevices) {
                if (bluetoothDevice.isConnected()) {
                    processBTDevice(localBtManager, sbArr, bluetoothDevice);
                }
            }
            for (int i = 0; i < 3; i++) {
        ...
        List<AppInfo> getAnomalyAppList() {
            return BatteryTipUtils.detectAnomalies(this.mContext, System.currentTimeMillis() - TimeUnit.HOURS.toMillis(new BatteryTipPolicy(this.mContext).appRestrictionActiveHour));
        }

        private Builder addTelephonyCursorSignals(String str) {
            Cursor query = this.mContext.getContentResolver().query(new Uri.Builder().scheme("content").authority("com.google.android.connectivitymonitor.troubleshooterprovider").path(str).build(), PsdBundle.TELEPHONY_PROJECTION, null, null, null);
            if (query == null) {
                if (query != null) {
                    query.close();
                }
