...
public final class ErrorCorrection {
    ...
    private ModulusPoly[] runEuclideanAlgorithm(ModulusPoly modulusPoly, ModulusPoly modulusPoly2, int i) throws ChecksumException {
        if (modulusPoly2.getDegree() < modulusPoly2.getDegree()) {
            modulusPoly2 = modulusPoly2;
            modulusPoly2 = modulusPoly2;
        }
        ...
        while (modulusPoly2.getDegree() >= i / 2) {
            if (!modulusPoly2.isZero()) {
                ...
                while (modulusPoly2.getDegree() >= modulusPoly2.getDegree() && !modulusPoly2.isZero()) {
                    int degree = modulusPoly2.getDegree() - modulusPoly2.getDegree();
                    ...
                    modulusPoly2 = modulusPoly2.subtract(modulusPoly2.multiplyByMonomial(degree, multiply));
                }
                zero = zero2.multiply(zero).subtract(zero).negative();
            } else {
                throw ChecksumException.getChecksumInstance();
            }
        }
        ...
        if (coefficient != 0) {
            ...
            return new ModulusPoly[]{zero.multiply(inverse2), modulusPoly2.multiply(inverse2)};
        }
        throw ChecksumException.getChecksumInstance();
    }
