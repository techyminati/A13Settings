...
public final class ReedSolomonDecoder {
    ...
    private GenericGFPoly[] runEuclideanAlgorithm(GenericGFPoly genericGFPoly, GenericGFPoly genericGFPoly2, int i) throws ReedSolomonException {
        if (genericGFPoly2.getDegree() < genericGFPoly2.getDegree()) {
            genericGFPoly2 = genericGFPoly2;
            genericGFPoly2 = genericGFPoly2;
        }
        GenericGFPoly zero = this.field.getZero();
        zero = this.field.getOne();
        while (genericGFPoly2.getDegree() >= i / 2) {
            if (!genericGFPoly2.isZero()) {
                GenericGFPoly zero2 = this.field.getZero();
                int inverse = this.field.inverse(genericGFPoly2.getCoefficient(genericGFPoly2.getDegree()));
                while (genericGFPoly2.getDegree() >= genericGFPoly2.getDegree() && !genericGFPoly2.isZero()) {
                    int degree = genericGFPoly2.getDegree() - genericGFPoly2.getDegree();
                    ...
                    genericGFPoly2 = genericGFPoly2.addOrSubtract(genericGFPoly2.multiplyByMonomial(degree, multiply));
                }
                zero = zero2.multiply(zero).addOrSubtract(zero);
                if (genericGFPoly2.getDegree() >= genericGFPoly2.getDegree()) {
        ...
        if (coefficient != 0) {
            ...
            return new GenericGFPoly[]{zero.multiply(inverse2), genericGFPoly2.multiply(inverse2)};
        }
